...
var RTCPeerConnectionEx = function(roomid, forceOptions) {
    ...
    function SocketConnection(connection, connectCallback) {
        ...
        if (connection.enableBroadcast) {
            parameters += '&enableBroadcast=true';
            parameters += '&maxRelayCountPerUser=' + (connection.maxRelayCountPerUser || 2);
        }
    ...
    function MultiPeers(connection) {
        ...
        this.getLocalConfig = function(remoteSdp, remoteUserId, userPreferences) {
            ...
            return {
                ...
                rtcPeerConnectionEx: connection,
                ...
                onPeerStateChanged: function(states) {
                    ...
                    if (states.iceConnectionState === 'new') {
                        console.log("states.iceConnectionState === new");
                        self.onNegotiationStarted(remoteUserId, states);
                    }

                    if (states.iceConnectionState === 'connected' ) {
                        console.log("states.iceConnectionState === connected  && states.signalingState === stable");
                        self.onNegotiationCompleted(remoteUserId, states);
                    }

                    if (states.iceConnectionState === 'completed' ) {
                        console.log("states.iceConnectionState === completed");
                    }

                    if (states.iceConnectionState.search(/closed|failed/gi) !== -1) {
                        console.log("states.iceConnectionState === closed|failed");

                        // self.onUserLeft(remoteUserId);
                        // self.disconnectWith(remoteUserId);

                        // johnny xu, 2021/4/26
                        connection.socket.emit('report-punchhole-status', states, remoteUserId, function() {});
                    }
                }
            };
        };
 ...
 //                   urls: 'stun:stun.l.google.com:19302'
    ...
    function getRandomString() {
        if (window.crypto && window.crypto.getRandomValues && navigator.userAgent.indexOf('Safari') === -1) {
			...
			console.debug('window.crypto.getRandomValues - token: ' + token);
            return token;
        } else {
			var token = (Math.random() * new Date().getTime()).toString(36).replace(/\./g, '');
			console.debug('Math.random() - token: ' + token);
            return token;
        }
    }
    ...
    function PeerInitiator(config) {
        ...
        var connection = config.rtcPeerConnectionEx;
        ...
        if (currentUserMediaRequest.streams[idInstance]) {
            ...
            if (isBlackBerry || typeof navigator.mediaDevices === 'undefined' || typeof navigator.mediaDevices.getUserMedia !== 'function') {
                ...
                navigator.getUserMedia(options.localMediaConstraints, function(stream) {
					...
					console.debug('stream.streamid = stream.streamid || stream.id || getRandomString(): ' + stream.streamid);
    ...
    var TextSender = {
        send: function(config) {
			...
			console.debug('TextSender - send() - var uuid = getRandomString(): ' + uuid);
    ...
    (function(connection) {
        ...
        connection.socketOptions = {
            ...
            'transport': ['websocket', 'polling'] //'polling' // fixing transport:unknown issues
        };
		...
		console.debug('connection.userid = getRandomString(): ' + connection.userid);
        connection.changeUserId = function(newUserId, callback) {
            callback = callback || function() {};
            connection.userid = newUserId || getRandomString();
			console.debug('connection.userid = newUserId || getRandomString(): ' + connection.userid);
		...
		console.debug('connection.token = getRandomString()');
        ...
        connection.socketURL = '/'; 
        connection.socketMessageEvent = 'Message-Event'; 
        connection.socketCustomEvent = 'Custom-Message-Event'; 
        ...
        connection.enableBroadcast = false;
        connection.maxRelayCountPerUser = 2; // each broadcast should serve only 3 users
...
if (typeof module !== 'undefined' /* && !!module.exports*/ ) {
    module.exports = exports = RTCPeerConnectionEx;
}

if (typeof define === 'function' && define.amd) {
    define('RTCPeerConnectionEx', [], function() {
        return RTCPeerConnectionEx;
    });
}
